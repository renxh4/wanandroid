...
class RenxhPlugin implements Plugin<Project>{
    ...
    private static void processArtifacts(Collection<ResolvedArtifact> artifacts) {
        ...
        for (final ResolvedArtifact artifact in artifacts) {
            ...
            printDebugmm("---------------[依赖产物开始] group=${gp}:${na}----------------")
           ZipFile zipFile =  new ZipFile(artifact.file)
            ...
            while (entries.hasMoreElements()){
                ...
                if (entry.isDirectory()){
                   ...
                   def name =  entry.name.substring(lastIndex)
                    if (name.endsWith(".so")){
                        printDebugmm("so文件 = "+ entry.name+"   size = "+(entry.getCompressedSize()/1024/1024))
            ...
            printDebugmm("---------------[依赖产物结束] group=${gp}:${na}----------------")
        }

        v8a.each {
            if (!arm.contains(it)){
                printDebugmm("arm不包含 = "+ it+"   "+ armMap.get(it))
            }
        }

        arm.each {
            if (!v8a.contains(it)){
                printDebugmm("v8a不包含 = "+ it+"   "+armMap.get(it))
            }
        }
    }
